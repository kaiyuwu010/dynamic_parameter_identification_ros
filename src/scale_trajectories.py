...
qs_des_ = []
with open("/home/thy/target_joint_states.csv", newline="") as csvfile:
        csv_reader = csv.DictReader(csvfile)
        # read csv and fill self.qs_des_
        for row in csv_reader:
            qs_des_.append([float(qi_des) for qi_des in list(row.values())[0:8]])
...
with open(output_path, "w") as csvfile:
    ...
    for i in range(1,len(qs_des_)):
        for j in range(10):
            value = [(np.asarray(item_1) + float(j)/10.0*(np.asarray(item)-np.asarray(item_1))).tolist() for item_1, item in zip(qs_des_[i-1],qs_des_[i])]
